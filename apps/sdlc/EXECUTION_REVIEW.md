...
6. After correction, inspect the new diff and evidence. Resolve comments yourself or reopen them. Agent completion never automatically resolves human comments. Accept when satisfied, recording an acceptance note if verification is incomplete.
...
- Review and patch export use the task's original base commit, including changes committed locally later. The patch exporter retains its existing text/size exclusions; unsupported changes need local Git inspection.
